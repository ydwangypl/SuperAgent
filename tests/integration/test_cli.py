...
class TestSuperAgentCLI(unittest.TestCase):
    ...
    def test_prompt_format(self):
        ...
        # 兼容小白模式和大神模式的提示符
        self.assertTrue(
            self.cli.prompt.endswith(">\033[0m ") or 
            "SuperAgent>" in self.cli.prompt
        )

    def test_intro_display(self):
        """测试欢迎信息"""
        intro = self.cli.intro
        self.assertIn("SuperAgent v3.2", intro)
        # 兼容不同模式下的欢迎词
        found = any(keyword in intro for keyword in ["自然语言编程系统", "你的 AI 编程小伙伴", "开发者专家系统"])
        self.assertTrue(found, f"Intro does not contain expected keywords: {intro}")
