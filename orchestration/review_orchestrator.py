...
class ReviewOrchestrator(BaseOrchestrator):
    ...
    def __init__(
        ...
        if REVIEW_AVAILABLE and self.config.enable_code_review:
            review_config = ReviewConfig(
                enable_style_check=self.config.enable_style_check,
                enable_security_check=self.config.enable_security_check,
                enable_performance_check=self.config.enable_performance_check,
                enable_best_practices=self.config.enable_best_practices,
                enable_ralph_wiggum=self.config.enable_ralph_wiggum,
    ...
    async def run_review(
        ...
        try:
            ...
            if self.ralph_wiggum_loop and self.config.enable_ralph_wiggum:
                review_result = await self._run_ralph_wiggum_review(project_id, files_to_review, code_files)
            else:
    ...
    async def _prepare_review_files(self, executed_tasks: List[TaskExecution]) -> tuple[Dict[str, str], List[Path]]:
        """准备待审查的文件内容和路径"""
        code_files = {}
        files_to_review = []
        read_tasks = []
        ...
        for res in read_results:
            if res:
                basename, content, full_path = res
    ...
    async def _run_ralph_wiggum_review(self, project_id: str, files: List[Path], code_content: Dict[str, str]) -> ReviewResult:
        """执行带改进循环的审查"""
    ...
    async def _improvement_callback(self, current_code: Dict[str, str], improvements: List[Dict]) -> Dict[str, str]:
        """改进回调函数 - 应用改进建议"""
    ...
    def _build_summary(self, review_result: ReviewResult) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'total_issues': review_result.metrics.issue_count,
            ...
            'meets_threshold': review_result.metrics.overall_score >= self.config.min_overall_score,
            'ralph_wiggum_enabled': self.ralph_wiggum_loop is not None and self.config.enable_ralph_wiggum
        }
