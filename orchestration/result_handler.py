...
from .models import TaskExecution, TaskStatus, ExecutionResult, OrchestrationState
...
class ExecutionResultHandler:
    ...
    def collect_results(self, executed_tasks: List[TaskExecution]) -> ExecutionResult:
        """汇总任务执行结果"""
    ...
    def update_state(self, executed_tasks: List[TaskExecution]) -> None:
        ...
        logger.debug(f"进度更新: {self.state.progress:.1f}% ({completed_count}/{self.state.total_tasks})")
